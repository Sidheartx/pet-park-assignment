...
contract PetPark {
    ...
    // Add an animal type
    function addAnimalType(uint256 _typeID, string memory _name, uint256 _default_count) public onlyOwner {
    ...
    function getAnimalData(uint256 _typeID) public view returns (petparkbase memory) {
    ...
    animalIdRef[_typeId].animal_count = animalIdRef[_typeId].animal_count + _count;
    ...
    function borrow(uint256 age, string memory gender, uint256 _typeId) public  {
        ...
        if (keccak256(bytes(gender)) == keccak256(bytes("male"))) {
            require(_validateMaleBorrow(_typeId), "Invalid animal type for male borrower");
        } else if (keccak256(bytes(gender)) == keccak256(bytes("female"))) {
            require(_validateFemaleBorrow(age, _typeId), "Invalid animal type for female borrower");
        } else {
            revert("Invalid gender provided");
        }
        
        require(animalIdRef[_typeId].animal_count > 0, "That animal breed is on the verge of extinction"); ///assuming valid Id has been entered by user
        animalIdRef[_typeId].animal_count = animalIdRef[_typeId].animal_count - 1;  
...
// give back aninmal, update user and animal database 
    function giveBackAnimal() public {
        require(userBorrowCount[msg.sender] == 1, "You haven't borrowed any animals yet"); 
        userBorrowCount[msg.sender] -= 1; 
        uint _userBorrowedAnimal = usersBorrowedAnimalTypeId[msg.sender]; 
        animalIdRef[_userBorrowedAnimal].animal_count = animalIdRef[_userBorrowedAnimal].animal_count + 1; 
   ...
   string[] memory maleBorrowableAnimals = new string[](2);
    ...
    for (uint256 i = 0; i < maleBorrowableAnimals.length; i++) {
        if (keccak256(bytes(animal_name)) == keccak256(bytes(maleBorrowableAnimals[i]))) {
            return false;
        }
    }
    return true;
}
    function _validateFemaleBorrow(uint256 age, uint256 _typeId) internal view returns (bool) {
    string memory disallowed_animal_name = "Cat";
    string memory animal_name = animalIdRef[_typeId].animal_name;

    if (age < 40 && keccak256(bytes(animal_name)) == keccak256(bytes(disallowed_animal_name))) {
